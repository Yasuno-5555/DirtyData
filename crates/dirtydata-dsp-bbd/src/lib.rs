...
impl BbdDelay {
    ...
    pub fn process(&mut self, input: f32, time_ms: f32, feedback: f32, dirt: f32) -> f32 {
        ...
        let filtered_input = Self::one_pole_lp(&mut self.aa_z2, filtered_input, aa_cutoff, self.sample_rate);
        
        // --- Clock jitter (BBD characteristic) ---
        self.phase_slow += two_pi * 0.7 / self.sample_rate;  // ~0.7 Hz drift
        if self.phase_slow >= two_pi { self.phase_slow -= two_pi; }
        
        self.phase_fast += two_pi * 5.5 / self.sample_rate;  // ~5.5 Hz flutter
        if self.phase_fast >= two_pi { self.phase_fast -= two_pi; }
        ...
        if read_idx_f < 0.0 { read_idx_f += buf_len as f32; }
        ...
        let filtered_delayed = Self::one_pole_lp(&mut self.recon_z1, delayed, recon_cutoff, self.sample_rate);
        let filtered_delayed = Self::one_pole_lp(&mut self.recon_z2, filtered_delayed, recon_cutoff, self.sample_rate);
        
        // --- Companding noise (BBD characteristic distortion) ---
        // Envelope follower for companding
        let abs_sig = filtered_delayed.abs();
        let comp_alpha = if abs_sig > self.comp_env { 0.01 } else { 0.0001 };
