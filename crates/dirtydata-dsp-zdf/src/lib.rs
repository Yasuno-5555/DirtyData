...
impl ZdfLadder {
    ...
    pub fn process_multi(&mut self, input: f32, cutoff_hz: f32, resonance: f32, drive: f32) -> LadderOutput {
        ...
        let input_2 = if drive > 0.01 { (lp[0] * (1.0 + drive * 0.25)).tanh() } else { lp[0] };
        v = (input_2 - self.s[1]) * g_val;
        lp[1] = v + self.s[1];
        self.s[1] = lp[1] + v;
        
        // Stage 3
        let input_3 = if drive > 0.01 { (lp[1] * (1.0 + drive * 0.25)).tanh() } else { lp[1] };
        v = (input_3 - self.s[2]) * g_val;
        lp[2] = v + self.s[2];
        self.s[2] = lp[2] + v;
        
        // Stage 4
        let input_4 = if drive > 0.01 { (lp[2] * (1.0 + drive * 0.25)).tanh() } else { lp[2] };
