...
impl BitCrushNode {
    pub fn new() -> Self { Self { inner: BitCrush::new() } }
}
...
impl DspNode for BitCrushNode {
    fn process(&mut self, inputs: &[f32], outputs: &mut [[f32; 2]], config: &ConfigSnapshot, _ctx: &ProcessContext) {
        ...
        let sr_div = config.get("sr_div").and_then(|v| v.as_float()).unwrap_or(1.0) as f32;
        
        for (i, output) in outputs.iter_mut().enumerate() {
            let input = if i * 2 < inputs.len() { inputs[i * 2] } else { 0.0 };
...
impl WaveShaperNode {
    pub fn new() -> Self { Self { inner: WaveShaper {} } }
}

impl DspNode for WaveShaperNode {
    fn process(&mut self, inputs: &[f32], outputs: &mut [[f32; 2]], config: &ConfigSnapshot, _ctx: &ProcessContext) {
        let amount = config.get("amount").and_then(|v| v.as_float()).unwrap_or(0.5) as f32;
...
impl PllNode {
    pub fn new() -> Self { Self { inner: Pll::new() } }
}

impl DspNode for PllNode {
    fn process(&mut self, inputs: &[f32], outputs: &mut [[f32; 2]], config: &ConfigSnapshot, ctx: &ProcessContext) {
        let lock_speed = config.get("lock_speed").and_then(|v| v.as_float()).unwrap_or(0.1) as f32;
