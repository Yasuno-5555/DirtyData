...
impl MidiInNode {
    pub fn new(event_rx: crossbeam_channel::Receiver<MidiEvent>) -> Self {
        Self { event_rx, gate: 0.0, pitch_hz: 440.0, velocity: 0.0, pending_events: Vec::new() }
    }
}

impl DspNode for MidiInNode {
    fn process(&mut self, _inputs: &[f32], outputs: &mut [[f32; 2]], _config: &ConfigSnapshot, ctx: &ProcessContext) {
        while let Ok(event) = self.event_rx.try_recv() { self.pending_events.push(event); }
        ...
        if outputs.len() > 1 { outputs[1] = [self.pitch_hz, self.pitch_hz]; }
        if outputs.len() > 2 { outputs[2] = [self.velocity, self.velocity]; }
    }
}
...
impl TriggerNode { pub fn new() -> Self { Self { last_gate: false } } }
...
pub enum EnvState { Idle, Attack, Decay, Sustain, Release, FastRelease }
...
impl EnvelopeNode { pub fn new() -> Self { Self { state: EnvState::Idle, level: 0.0 } } pub fn is_idle(&self) -> bool { self.state == EnvState::Idle } }
impl DspNode for EnvelopeNode {
    fn process(&mut self, inputs: &[f32], outputs: &mut [[f32; 2]], config: &ConfigSnapshot, ctx: &ProcessContext) {
        ...
        let gate = inputs.get(0).cloned().unwrap_or(0.0) > 0.0;
        match self.state {
            EnvState::Idle => { if gate { self.state = EnvState::Attack; } }
            EnvState::Attack => {
            ...
            EnvState::Decay => {
                if !gate { self.state = EnvState::Release; }
                    ...
                    if self.level <= s { self.level = s; self.state = EnvState::Sustain; }
                }
            }
            EnvState::Sustain => { if !gate { self.state = EnvState::Release; } }
            EnvState::Release => {
    ...
    fn update_parameter(&mut self, param: &str, _value: f32) { if param == "steal" { self.state = EnvState::FastRelease; } }
    fn extract_state(&self) -> NodeState { NodeState::from_json(serde_json::json!({ "state": self.state, "level": self.level })) }
    fn inject_state(&mut self, state: &NodeState) {
        if let Some(data) = state.to_json::<serde_json::Value>() {
            if let Some(s) = data.get("state").and_then(|v| serde_json::from_value::<EnvState>(v.clone()).ok()) { self.state = s; }
            if let Some(l) = data.get("level").and_then(|v| v.as_f64()) { self.level = l as f32; }
        }
    }
}
...
impl AutomationNode { pub fn new() -> Self { Self { smooth: SmoothedValue::new(0.0, 44100.0, 10.0) } } }
impl DspNode for AutomationNode {
    fn process(&mut self, _inputs: &[f32], outputs: &mut [[f32; 2]], config: &ConfigSnapshot, _ctx: &ProcessContext) {
        let val = config.get("value").and_then(|v| v.as_float()).unwrap_or(0.0) as f32;
...
impl SequencerNode { pub fn new() -> Self { Self { last_step_idx: -1 } } }
