    ...
    pub fn new(sample_rate: f32, definition_json: &str) -> Option<Self> {
        ...
        for el in &elements {
            let nodes = match el {
                ...
                dirtydata_dsp_circuit::CircuitElement::CurrentSource { pos: a, neg: b, .. } => vec![*a, *b],
                ...
                dirtydata_dsp_circuit::CircuitElement::Transformer { a1, b1, a2, b2, .. } => vec![*a1, *b1, *a2, *b2],
                dirtydata_dsp_circuit::CircuitElement::OpAmp { pos, neg, out, .. } => vec![*pos, *neg, *out],
                dirtydata_dsp_circuit::CircuitElement::Potentiometer { a, wiper, b, .. } => vec![*a, *wiper, *b],
                dirtydata_dsp_circuit::CircuitElement::ControlledSource { target_a, target_b, control_a, control_b, .. } => vec![*target_a, *target_b, *control_a, *control_b],
                dirtydata_dsp_circuit::CircuitElement::TransmissionLine { a1, b1, a2, b2, .. } => vec![*a1, *b1, *a2, *b2],
