...
use dirtydata_core::types::{StableId, ConfigSnapshot, NodeKind};
use std::collections::HashMap;
use wasmtime::*;
use wasm_encoder::{
    CodeSection, ExportSection, Function, FunctionSection, MemorySection,
    Module as WasmModule, TypeSection, ValType, Instruction,
};
...
impl JitProgram {
    pub fn new(
        ...
        let data = JitStoreData { legacy_nodes, configs, sample_rate: 44100.0, global_sample_index: 0 };
        let mut store = Store::new(engine, data);
        let module = Module::new(engine, wasm_bytes)?;
        let mut linker = Linker::new(engine);
        
        linker.func_wrap("host", "sin", |_: Caller<'_, JitStoreData>, x: f32| x.sin())?;
        linker.func_wrap("host", "tanh", |_: Caller<'_, JitStoreData>, x: f32| x.tanh())?;
        
        linker.func_wrap("host", "call_legacy", |mut caller: Caller<'_, JitStoreData>, node_idx: i32, in_ptr: i32, in_len: i32, out_ptr: i32, out_len: i32| {
            ...
            for i in 0..in_len as usize {
                ...
                if offset + 4 <= mem_data.len() { inputs[i] = f32::from_le_bytes(mem_data[offset..offset+4].try_into().unwrap()); }
            }
            let ctx = ProcessContext {
                sample_rate: caller.data().sample_rate, global_sample_index: caller.data().global_sample_index,
                crash_flag: None, osc_tx: None, convergence_info: None, node_diagnostics: None, node_id: None,
            };
            let (node, config) = { let d = caller.data_mut(); (&mut d.legacy_nodes[node_idx as usize], &d.configs[node_idx as usize]) };
            ...
            for i in 0..out_len as usize {
                ...
                if offset + 8 <= mem_data.len() {
                    ...
                    mem_data[offset+4..offset+8].copy_from_slice(&outputs[i][1].to_le_bytes());
                }
            }
        })?;

        // NATIVE MNA PATH
        linker.func_wrap("host", "call_mna", |mut caller: Caller<'_, JitStoreData>, node_idx: i32, in_ptr: i32, in_len: i32, out_ptr: i32, out_len: i32| {
            let mem = match caller.get_export("memory") { Some(Extern::Memory(m)) => m, _ => return };
            let mut inputs = vec![0.0f32; in_len as usize];
            let mem_data = mem.data(&caller);
            for i in 0..in_len as usize {
            ...
            node.process(&inputs, &mut outputs, &data.configs[node_idx as usize], &ProcessContext {
                sample_rate: data.sample_rate, global_sample_index: data.global_sample_index,
                crash_flag: None, osc_tx: None, convergence_info: None, node_diagnostics: None, node_id: None,
            });
        ...
        let process_fn = instance.get_typed_func::<(f32, u64), (f32, f32)>(&mut store, "process")?;
        Ok(Self { store, instance, process_fn })
    }
    ...
    pub fn execute(&mut self, ctx: &ProcessContext) -> [f32; 2] {
        ...
        match self.process_fn.call(&mut self.store, (ctx.sample_rate, ctx.global_sample_index)) {
            Ok((l, r)) => [l, r],
            Err(e) => { tracing::error!("JIT execution error: {}", e); [0.0, 0.0] }
        }
    }
}

pub struct JitCompiler { engine: Engine, register_map: HashMap<StableId, u32>, state_map: HashMap<StableId, u32>, next_local: u32, next_state_offset: u32 }

impl JitCompiler {
    pub fn new() -> Self {
        let mut config = Config::new(); config.cranelift_opt_level(OptLevel::Speed);
        let engine = Engine::new(&config).unwrap();
        Self { engine, register_map: HashMap::new(), state_map: HashMap::new(), next_local: 2, next_state_offset: 0 }
    }
    ...
    pub fn compile_runner(&mut self, runner: &crate::DspRunner) -> anyhow::Result<JitProgram> {
        ...
        types.ty().function(vec![ValType::F32, ValType::I64], vec![ValType::F32, ValType::F32]);
        types.ty().function(vec![ValType::I32, ValType::I32, ValType::I32, ValType::I32, ValType::I32], vec![]);

        memory.memory(wasm_encoder::MemoryType { minimum: 2, maximum: None, memory64: false, shared: false, page_size_log2: None });
        ...
        let scratch_in = 65536; let scratch_out = 65536 + 1024;

        for (id, _) in &runner.nodes {
            self.register_map.insert(*id, self.next_local); self.next_local += 1;
            if let Some(node_ir) = graph.nodes.get(id) {
                if let Some(name) = node_ir.config.get("name").and_then(|v| v.as_string()) {
                    if name == "Oscillator" { self.state_map.insert(*id, self.next_state_offset); self.next_state_offset += 4; }
                }
            }
        }
        ...
        for (id, node_impl) in &runner.nodes {
            ...
            if let Some(node_ir) = graph.nodes.get(id) {
                ...
                match name.as_deref().map(|s| s.as_str()) {
                    Some("Oscillator") => {
                        ...
                        func.instruction(&Instruction::F32Load(wasm_encoder::MemArg { offset: 0, align: 2, memory_index: 0 }));
                        ...
                        func.instruction(&Instruction::F32Store(wasm_encoder::MemArg { offset: 0, align: 2, memory_index: 0 }));
                    }
                    Some("Gain") => {
                        natively_lowered = true;
                    ...
                    Some("Add") => {
                        ...
                        if in_regs.is_empty() { func.instruction(&Instruction::F32Const(0.0)); } else {
                            func.instruction(&Instruction::LocalGet(in_regs[0]));
                            for &r in &in_regs[1..] { func.instruction(&Instruction::LocalGet(r)); func.instruction(&Instruction::F32Add); }
                        }
                        func.instruction(&Instruction::LocalSet(out_reg));
                    }
                    Some("Multiply") => {
                        natively_lowered = true;
                        let in_regs = self.find_all_input_regs(graph, id);
                        if in_regs.len() < 2 { func.instruction(&Instruction::F32Const(0.0)); } else {
                            func.instruction(&Instruction::LocalGet(in_regs[0]));
                            for &r in &in_regs[1..] { func.instruction(&Instruction::LocalGet(r)); func.instruction(&Instruction::F32Mul); }
                        }
                        func.instruction(&Instruction::LocalSet(out_reg));
                    }
                    _ if node_ir.kind == NodeKind::Sink => { natively_lowered = true; func.instruction(&Instruction::LocalGet(self.find_input_reg(graph, id))); func.instruction(&Instruction::LocalSet(out_reg)); }
                    _ => {}
                }
            }
            ...
            if !natively_lowered {
                ...
                let name = graph.nodes.get(id).unwrap().config.get("name").and_then(|v| v.as_string());
        ...
        if let Some((last_id, _)) = runner.nodes.last() {
            ...
            func.instruction(&Instruction::LocalGet(last_reg)); func.instruction(&Instruction::LocalGet(last_reg));
        } else { func.instruction(&Instruction::F32Const(0.0)); func.instruction(&Instruction::F32Const(0.0)); }
        func.instruction(&Instruction::End);

        functions.function(1); code.function(&func);
        module.section(&types); module.section(&imports); module.section(&functions); module.section(&memory); module.section(&exports); module.section(&code);
    ...
    fn find_input_reg(&self, graph: &dirtydata_core::ir::Graph, node_id: &StableId) -> u32 {
        for edge in graph.edges.values() { if edge.target.node_id == *node_id { if let Some(&src) = self.register_map.get(&edge.source.node_id) { return src; } } }
        0
    }
    fn find_all_input_regs(&self, graph: &dirtydata_core::ir::Graph, node_id: &StableId) -> Vec<u32> {
        let mut regs = Vec::new();
        for edge in graph.edges.values() { if edge.target.node_id == *node_id { if let Some(&src) = self.register_map.get(&edge.source.node_id) { regs.push(src); } } }
        regs
    }
}
