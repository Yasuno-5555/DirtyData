...
use crate::types::*;
use dirtydata_dsp_circuit::{MnaSolver, CircuitElement};

impl crate::mutation::MutationEngine {
    /// Evaluates a mutated circuit by running a micro-simulation.
    pub fn evaluate(&self, def: &CircuitDefinition, _intensity: MutationIntensity) -> MutationReport {
        // 1. Setup a micro-solver (Impulse Response test)
        let mut solver = MnaSolver::new(1.0 / 44100.0);
        let elements: Vec<CircuitElement> = serde_json::from_str(&def.elements_json).unwrap_or_default();
        ...
        for i in 0..256 {
            ...
            if state.converged { converged_count += 1; }
        ...
        // 3. Compute Metrics
        let instability = if peak > 10.0 { 1.0 } else { (256 - converged_count) as f32 / 256.0 };
        let novelty = (def.mutation_history.len() as f32 * 0.1).min(1.0);
        let warmth = if peak > 0.0 && energy / peak > 50.0 { 0.8 } else { 0.2 };
